...
static void on_solve_back_button_clicked(GtkButton *button, gpointer menu_window) {
    gtk_widget_hide(GTK_WIDGET(gtk_widget_get_toplevel(GTK_WIDGET(button))));
    gtk_widget_show_all(GTK_WIDGET(menu_window));
    in_main_window = 1;
    in_solve_window = 0;
...
static void get_the_grid(GtkWidget *widget) {
    ...
    for (int row = 0; row < 9; row++) {
        for (int col = 0; col < 9; col++) {
            ...
            gtk_grid_attach(GTK_GRID(sudokuGrid), number_button, col, row, 1, 1);
        }
    }
...
static void on_filter_button_clicked(GtkWidget *widget, gpointer data) {
    ...
    // Apply the filter to the image (assuming 'data' is the path to the filter image)
    char *image_path = (char *)data;
    GdkPixbuf *pixbuf = gdk_pixbuf_new_from_file_at_scale(image_path, 600, 600, TRUE, NULL);
    if (pixbuf) {
    ...
    gtk_widget_set_sensitive(reducenoise_button, widget != reducenoise_button);
    ...
    gtk_widget_set_sensitive(hough_average_button, widget != hough_average_button);
    gtk_widget_set_sensitive(drawsquare_button, widget != drawsquare_button);
    gtk_widget_set_sensitive(extracted_button, widget != extracted_button);
...
void activate(GtkApplication* app, gpointer user_data) {
    ...
    back_preprocess_button = GTK_WIDGET(gtk_builder_get_object(builder2, "back_button"));
    ...
    if (s_button != NULL && GTK_IS_BUTTON(s_button)) {
        g_signal_connect(s_button, "clicked", G_CALLBACK(on_solve_button_clicked), solve_window);
    }

    back_s_button = GTK_WIDGET(gtk_builder_get_object(solve_builder, "back_button"));

    if (back_s_button != NULL && GTK_IS_BUTTON(back_s_button)) {
        g_signal_connect(back_s_button, "clicked", G_CALLBACK(on_solve_back_button_clicked), window1);
    }
