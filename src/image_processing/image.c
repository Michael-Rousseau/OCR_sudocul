...
void surface_to_blackwhite(SDL_Surface *surface) {
  ...
  for (int y = 0; y < surface->h; y++) {
    for (int x = 0; x < surface->w; x++) {
      Uint8 local_threshold = fast_local_threshold(
          integral, x, y, neighborhood_size, copy->w, copy->h);
...
float **generate_Kernel(int ksize, float sigma) {
  ...
  for (int i = 0; i < ksize; i++) {
    kernel[i] = (float *)calloc(ksize, sizeof(float));
  } // malloc
    ...
    for (int x = 0; x < imageWidth; x++) {
      ...
      for (int ky = 0; ky < kernelsize; ky++) {
        for (int kx = 0; kx < kernelsize; kx++) {
          ...
          if (Xf >= 0 && Xf < imageWidth && Yf < imageHeight && Yf >= 0) {
      ...
      Uint32 newPixelValue = SDL_MapRGBA(filteredimage->format, (Uint8)finalr,
                                         (Uint8)finalg, (Uint8)finalb, 255);
...
void dilation(SDL_Surface *image) {
      ...
      if (White(pixeli, image->format) == 1) {
        ...
        for (int ys = -centerkernel; ys < centerkernel + 1; ys++) {
          for (int xs = -centerkernel; xs < centerkernel + 1; xs++) {
            ((Uint32 *)
                 outputimage->pixels)[(y + ys) * outputimage->w + (x + xs)] =
  ...
  memcpy(image->pixels, outputimage->pixels,
         image->w * image->h * sizeof(Uint32));
...
void erosion(SDL_Surface *image) {
  ...
  for (int y = centerkernel; y < himage - centerkernel; y++) {
    for (int x = centerkernel; x < wimage - centerkernel; x++) {
      ...
      for (int ys = -centerkernel; ys < centerkernel + 1; ys++) {
        if (black == 0) {
          for (int xs = -centerkernel; xs < centerkernel + 1; xs++) {
            if (black == 0) {
              Uint32 pixeli =
                  ((Uint32 *)image->pixels)[(y + ys) * wimage + (xs + x)];
      ...
      if (black == 0)
        ((Uint32 *)outputimage->pixels)[y * outputimage->w + x] =
            SDL_MapRGB(image->format, 255, 255, 255);
    }
  }
  memcpy(image->pixels, outputimage->pixels,
    ...
    for (int x = 1; x < imagew - 1; x++) {
      ...
      for (int ky = -1; ky <= 1; ky++) {
        for (int kx = -1; kx <= 1; kx++) {
          ...
          Uint8 intensity;
          SDL_GetRGB(neighborPixel, image->format, &intensity, &intensity,
                     &intensity);
...
double *nonMaximalSuppressionAndHysteresis(double *gradient, double *ang,
...
void Canny_edge_result(SDL_Surface *image) {
  ...
  double *edge = nonMaximalSuppressionAndHysteresis(
      gradient, ang, image->w, image->h, maxGradient * 0.35, maxGradient * 0.7);
