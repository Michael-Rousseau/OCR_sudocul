...
int thelinebelongs(struct Line l1, struct Line l2) {
  ...
  // if (fabs(l1.rho - l2.rho) <= rhoThreshold && fabs(l1.theta - l2.theta) <=
  // thetaThreshold)
...
void drawl(struct Line *line, int len, SDL_Renderer *renderer) {
  struct Linegroup **group = calloc(
      len,
      sizeof(
          struct Linegroup *)); // use len as it represents the number of lines
  if (!group) {
    fprintf(stderr, "Memory allocation failed.\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (int i = 0; i < nmax; i++) {
    ...
    SDL_RenderDrawLine(renderer, group[i]->average.start.x,
                       group[i]->average.start.y, group[i]->average.end.x,
                       group[i]->average.end.y);
  }
  for (int i = 0; i < nmax; i++)
    free(group[i]);
  free(group);
...
{
    if (fabs(l1.rho - l2.rho) < rhoThreshold && fabs(l1.theta - l2.theta) <
thetaThreshold) return 1; return 0;
}

void addtogroup(struct Line line, struct Linegroup*  group)
{
        group->average.start.x =(group->average.start.x * group.numlines +
line.start.x) / (group.numlines + 1); group->average->star
=(group->average->start->y * group->numlines + line->start->y) /
(group->numlines + 1); group->average.end- =(group->average.end * group.numlines
+ line.end) / (group.numlines + 1); group->average->end->y
=(group->average->end->y * group->numlines + line->end->y) / (group->numlines +
1); group->numlines +=1;
}


int averagelines( struct Line* line,int len, SDL_Surface* image, struct
Linegroup** group)
{
        ...
        for(int i = 0; i < len; i++)
        {
                ...
                if (loner)
                {
                        ...
                        {
                                n*=2;
                                group = realloc(group, n* sizeof(struct
Linegroup*));
                        }
                        group[ncurrent] = calloc(1, sizeof(struct Linegroup));
...
{	struct Linegroup** group = calloc(n,sizeof(struct Linegroup*));
        ...
        int nmax = averagelines(line, len, image,group);
                ...
                for(int i = 0; i < nmax; i++)
                {
                        ...
                        SDL_SetRenderDrawColor(renderer, 255, 0, 0,
SDL_ALPHA_OPAQUE); // Red color for the lines SDL_RenderDrawLine(renderer,
group[i]->average.start.x, group[i]->average.start.y,group[i]->average.end.x,
group[i]->average.end.y);
                }
}*/
