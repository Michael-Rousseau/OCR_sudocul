...
void back_prop(network *n, int* expected) {
    ...
    for (size_t i = 0; i < NUM_THREADS; i++) {
        ...
        threadData[i].end = current_start + neurons_per_thread + (i < remaining_neurons ? 1 : 0);
    ...
    for (size_t i = start; i < end; i++) {
        for (size_t j = 0; j < n->layers[i]; j++) {
            for (size_t k = 0; k < n->layers[i + 1]; k++) {
                n->weights[i][k][j] -= data->speed * n->values[i][j] * n->costs[i][k];
            }
        }

        for (size_t j = 0; j < n->layers[i + 1]; j++) {
            n->biases[i][j] -= data->speed * n->costs[i][j];
        }
    }
...
void learn(network *n, double speed) {
    ...
    size_t weights_per_thread = n->layers[last - 1] / NUM_THREADS;
    size_t remaining_weights = n->layers[last - 1] % NUM_THREADS;
    size_t current_start = 0;

    for (size_t i = 0; i < NUM_THREADS; i++) {
        threadData[i].n = n;
        threadData[i].start = current_start;
        threadData[i].end = current_start + weights_per_thread + (i < remaining_weights ? 1 : 0);
        threadData[i].mutex = &mutex;
