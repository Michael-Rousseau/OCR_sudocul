...
void train(network *n, double speed, double **inputs, int **targets,
           size_t num_samples, size_t num_epochs, size_t batch_size, int label_first[]) {
  load_mnist();
...
void load_and_train(network *n) {
  ...
  train(n, LEARNING_RATE, inputs, targets, num_samples, num_epochs, batch_size, label_first);
  ...
  train(n, LEARNING_RATE, inputs, targets, num_samples, num_epochs, batch_size, label_second);
...
void test_from_file() {
  network *n = import_network("networkOwnData.nw");
  const char *filename = "./data/tst/firstimg/square_1.bmp";
